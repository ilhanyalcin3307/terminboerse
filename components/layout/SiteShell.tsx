...
import { BriefcaseMedical, Clock3, LogIn, Wrench, X } from "lucide-react";
        ...
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              type="button"
              aria-disabled="true"
              title="Arztbereich ist bald verfuegbar"
              className="inline-flex cursor-not-allowed items-center gap-1.5 rounded-full border border-slate-200 bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-500 opacity-80 sm:text-sm"
            >
              <BriefcaseMedical className="h-4 w-4" />
              <span className="hidden sm:inline">Arztbereich (bald)</span>
            </button>

            <Link
              href="/kontakt?intent=login"
              onClick={() =>
                trackEvent("cta_clicked", {
                  source: "navbar-login",
                  action: "login_interest",
                  category: "auth",
                  district: "all",
                })
              }
            ...
            >
              ...
              <span className="hidden sm:inline">Termin anbieten</span>
              <span className="sm:hidden">Anbieten</span>
            </button>
          </div>
        </div>
      </header>
